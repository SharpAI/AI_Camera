...
extern "C"  {
    ...
    Java_org_dp_facedetection_LibFaceDetection_facedetect(JNIEnv *env,jobject /* this */,jlong matAddr)
    {
        jobjectArray faceArgs = nullptr;
        Mat& img  = *(Mat*)matAddr;
        Mat bgr = img.clone();
        cvtColor(img, bgr, COLOR_RGBA2BGR);
        __android_log_print(ANDROID_LOG_VERBOSE, JNITag,"convert RGBA to RGB");
        ...
        __android_log_print(ANDROID_LOG_VERBOSE, JNITag,"%d faces detected.\n", numOfFaces);
        ...
        for(int i = 0; i < (pResults ? *pResults : 0); i++)
        {
            ...
            int confidence = p[4];
            int angle = p[5];
            __android_log_print(ANDROID_LOG_VERBOSE, JNITag,"face_rect=[%d, %d, %d, %d], confidence=%d, angle=%d\n", x,y,w,h,confidence, angle);
